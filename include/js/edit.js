...
function loadNote(id) {
	updateStatusBar("#f1c40f", "页面加载中...");
	if (NOTEID) {
		if (NOTEID == id) {
			updateStatusBar("#0f2", "页面加载完毕");
			return 0;
		}
...
function getNoteSettings(id) {
	updateStatusBar("#f1c40f", "加载属性...");
	$.post("include/note.php", {
		action: "getNoteSettings",
		id: id
	},
		function (data, status) {
			updateStatusBar("#0f2", "属性加载完毕");
